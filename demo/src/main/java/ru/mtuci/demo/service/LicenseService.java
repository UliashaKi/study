...
public class LicenseService {
    ...
    public Ticket activateLicense(ActivateLicenseRequest req) throws APIException {
        ...
        //нашли запись связанного с устройством лицензии
        DeviceLicense existingDeviceLicense = deviceLicenseService.getDeviceLicenseByDeviceIdAndLicenseId(device.getId(), license.getId());

        //1. Проверить, не пустая ли deviceLicense
        if (existingDeviceLicense != null) {
            //2. Если не пустая, то выбросить исключение без catch, иначе идём дальше
